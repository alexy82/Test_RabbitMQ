...
Xóa exchange **test**
```bash
python command/exchange.py -a remove -e test
```
```bash
python command/exchange.py --action remove --exchange test
```
Tạo exchange từ file **exchanges.py**
```bash
python command/exchange.py -f /vagrant/code/exchanges.py
```
```bash
python command/exchange.py --file /vagrant/code/exchanges.py --action declare
```
...
Xóa queue **test**
```bash
python command/exchange.py -a remove -q test
```
```bash
python command/exchange.py --action remove --queue test
```
xóa queue **test** nếu không có consumer connect đến
```bash
python command/exchange.py --action remove --queue test -u
```
